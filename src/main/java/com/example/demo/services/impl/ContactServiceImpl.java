...
import com.example.demo.repsitiories.ContactRepo;
...
@Override
public Contact save(Contact contact) {
    // Use only for creating new contact
...
@Override
public Contact update(Contact contact) {
    // This assumes the contact already has an ID and exists
    return contactRepo.save(contact);
}
    ...
    @Override
    public Contact getByIdAndUser(String contactId, User user) {
        return contactRepo.findByIdAndUser(contactId, user)
            .orElseThrow(() -> new RuntimeException("Contact not found or does not belong to user"));
    }
    @Override
public Contact update(Contact contact, User user) {
    Contact existing = getByIdAndUser(contact.getId(), user);  // Verifies ownership
    contact.setUser(existing.getUser()); // Preserve original owner
    return contactRepo.save(contact);
}
